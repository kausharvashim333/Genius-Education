...
function renderEntranceResultHtml(r) {
    ...
    return '<div id="entResultPdfArea" style="background:#fff;color:' + s.textColor + ';padding:10px;font-family:\'Arial\',\'Helvetica\',sans-serif;width:210mm;height:297mm;box-sizing:border-box;position:relative;overflow:hidden;">' +
        watermarkHtml +
        '<div style="border:2px solid ' + s.headerColor + ';border-radius:8px;padding:15px;height:100%;position:relative;z-index:1;background:#fff;">' +
        
        // Header Section
        '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;padding-bottom:10px;border-bottom:2px solid ' + s.headerColor + ';">' +
        '<div style="flex:0 0 70px;">' + logoHtml + '</div>' +
        '<div style="flex:1;text-align:center;padding:0 10px;">' +
        '<h1 style="margin:0;font-size:22px;color:' + s.headerColor + ';font-weight:bold;letter-spacing:0.5px;text-transform:uppercase;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">' + s.instituteName + '</h1>' +
        '<p style="margin:5px 0 0 0;color:' + s.textColor + ';font-size:11px;font-weight:600;letter-spacing:0.3px;">' + s.tagline + '</p>' +
        (s.address ? '<p style="margin:3px 0 0 0;color:' + s.textColor + ';font-size:10px;opacity:0.8;">' + s.address + '</p>' : '') +
        '</div>' +
        '<div style="flex:0 0 70px;"></div>' +
        '</div>' +
        
        // Title Section
        '<div style="text-align:center;margin-bottom:15px;padding:10px;background:linear-gradient(135deg,' + s.headerColor + '15,' + s.headerColor + '8);border-radius:6px;border:1px solid ' + s.headerColor + ';">' +
        '<h2 style="margin:0;color:' + s.headerColor + ';font-size:16px;font-weight:bold;letter-spacing:0.8px;text-transform:uppercase;">OFFICIAL EXAMINATION SCORECARD</h2>' +
        '</div>' +
        
        // Student Details Table
        '<div style="margin-bottom:15px;">' +
        '<h3 style="margin:0 0 8px 0;color:' + s.headerColor + ';font-size:12px;font-weight:bold;letter-spacing:0.4px;border-left:3px solid ' + s.headerColor + ';padding-left:8px;">CANDIDATE INFORMATION</h3>' +
        '<table style="width:100%;border-collapse:collapse;border:1px solid #e5e7eb;border-radius:6px;overflow:hidden;">' +
        '<tr style="background:linear-gradient(135deg,' + s.headerColor + ',' + s.headerColor + '85);"><td style="padding:8px 10px;border:1px solid ' + s.headerColor + ';font-weight:bold;color:#fff;width:38%;font-size:11px;">Registration Number</td><td style="padding:8px 10px;border:1px solid ' + s.headerColor + ';color:#fff;font-weight:bold;font-size:12px;">' + (r.registrationNo || '') + '</td></tr>' +
        '<tr style="background:#fff;"><td style="padding:8px 10px;border:1px solid #e5e7eb;font-weight:bold;color:' + s.textColor + ';font-size:11px;">Student Name</td><td style="padding:8px 10px;border:1px solid #e5e7eb;font-size:12px;color:' + s.textColor + ';text-transform:uppercase;font-weight:600;">' + (r.studentName || '') + '</td></tr>' +
        '<tr style="background:#f9fafb;"><td style="padding:8px 10px;border:1px solid #e5e7eb;font-weight:bold;color:' + s.textColor + ';font-size:11px;">Applied Course</td><td style="padding:8px 10px;border:1px solid #e5e7eb;font-size:12px;color:' + s.textColor + ';text-transform:uppercase;font-weight:600;">' + (r.course || '-') + '</td></tr>' +
        '<tr style="background:#fff;"><td style="padding:8px 10px;border:1px solid #e5e7eb;font-weight:bold;color:' + s.textColor + ';font-size:11px;">Examination</td><td style="padding:8px 10px;border:1px solid #e5e7eb;font-size:12px;color:' + s.textColor + ';text-transform:uppercase;font-weight:600;">' + (r.examName || '') + '</td></tr>' +
        '<tr style="background:#f9fafb;"><td style="padding:8px 10px;border:1px solid #e5e7eb;font-weight:bold;color:' + s.textColor + ';font-size:11px;">Date of Examination</td><td style="padding:8px 10px;border:1px solid #e5e7eb;font-size:12px;color:' + s.textColor + ';">' + (r.submittedAt ? new Date(r.submittedAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'long', year: 'numeric' }) : '-') + '</td></tr>' +
        '</table>' +
        '</div>' +
        
        // Score Section
        '<div style="background:linear-gradient(135deg,' + s.headerColor + '8,' + s.headerColor + '3);border:1px solid ' + s.headerColor + ';border-radius:6px;padding:12px;margin-bottom:15px;">' +
        '<h3 style="margin:0 0 10px 0;color:' + s.headerColor + ';font-size:13px;font-weight:bold;letter-spacing:0.4px;text-align:center;border-bottom:1px solid ' + s.headerColor + ';padding-bottom:8px;">PERFORMANCE SUMMARY</h3>' +
        '<table style="width:100%;border-collapse:collapse;">' +
        '<tr style="background:#fff;"><td style="padding:10px;border:1px solid #e5e7eb;font-weight:bold;color:' + s.textColor + ';width:42%;font-size:11px;">Total Marks Obtained</td><td style="padding:10px;border:1px solid #e5e7eb;font-size:18px;font-weight:bold;color:' + s.headerColor + ';background:#fff;">' + r.marksObtained + ' <span style="font-size:13px;color:#6b7280;">/ ' + r.totalMarks + '</span></td></tr>' +
        '<tr style="background:#f9fafb;"><td style="padding:10px;border:1px solid #e5e7eb;font-weight:bold;color:' + s.textColor + ';font-size:11px;">Overall Percentage</td><td style="padding:10px;border:1px solid #e5e7eb;font-size:18px;font-weight:bold;color:' + s.headerColor + ';background:#f9fafb;">' + r.percentage + '<span style="font-size:13px;color:#6b7280;">%</span></td></tr>' +
        '</table>' +
        '</div>' +
        
        // Footer Section
        '<div style="margin-top:auto;padding-top:12px;border-top:1px solid ' + s.headerColor + ';display:flex;justify-content:space-between;align-items:flex-start;gap:3%;">' +
        '<div style="width:72%;box-sizing:border-box;">' +
        '<p style="margin:0 0 6px 0;font-size:10px;color:' + s.textColor + ';font-weight:600;letter-spacing:0.4px;">DOCUMENT INFORMATION</p>' +
        '<p style="margin:0;font-size:9px;color:' + s.textColor + ';opacity:0.8;">This is a computer-generated document. No manual signatures are required. This scorecard is valid for admission purposes only.</p>' +
        instructionsHtml +
        '</div>' +
        '<div style="width:25%;text-align:center;box-sizing:border-box;">' +
        '<p style="margin:0 0 6px 0;font-size:10px;color:' + s.textColor + ';font-weight:600;letter-spacing:0.4px;">AUTHORIZATION</p>' +
        '<div style="width:70px;height:40px;border-bottom:2px solid ' + s.headerColor + ';margin:6px auto 0 auto;"></div>' +
        '<p style="margin:4px 0 0 0;font-size:9px;color:' + s.textColor + ';font-weight:600;">' + s.signatureLabel + '</p>' +
        courseButtonsHtml +
        '</div>' +
        '</div>' +
