...
- Run `npm test` for isolated lead assignment, faculty session, and batch attendance tests. Fixtures use in-memory data and stub email delivery; tests do not load `.env`, start the full application, or modify `data/`.
- Run `npm run test:browser` on macOS/Linux for the optional Puppeteer lead and batch attendance workflow tests. It requires the existing Puppeteer dependency and its installed Chrome browser. External network requests are blocked in this test.
