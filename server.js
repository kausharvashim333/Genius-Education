...
// Generate Admission Form PDF (uses generateSlipHTML + Puppeteer)
async function generateAdmissionPDF(student, settings) {
    try {
        const html = generateSlipHTML(student, settings, null, '');
...
async function sendLoginCredentials(student, password) {
    ...
    // Resolve course + batch info
    const courses = readData('courses.json') || [];
    const batches = readData('batches.json') || [];
    const courseInfo = (Array.isArray(courses) ? courses : []).find(c => c.name === student.course) || {};
    const batchInfo = (Array.isArray(batches) ? batches : []).find(b => b.name === student.batch && b.course === student.course) || {};

    // Application ID (consistent with print form / QR)
    const year = new Date().getFullYear();
    ...
    const addr = settings.address || '';
    const phones = Array.isArray(settings.phone) ? settings.phone : (settings.phone ? [settings.phone] : []);
    const phonesHtml = phones.map(p => `<a href="tel:${p.replace(/\s+/g,'')}" style="color:#3b82f6;text-decoration:none;">${p}</a>`).join(' &nbsp;|&nbsp; ');
...
// Student self-service: upload/replace own document (session-verified)
app.post('/api/student/:id/documents', uploadStudentDoc.single('document'), (req, res) => {
    ...
    logActivity('student.document.upload', { type: 'student', id: student.id, name: student.name }, {
        studentId: student.id, docType, docLabel, uploadedBy: newDoc.uploadedBy, selfUpload: true
    }, req);
...
// Admission application verification page (for admission form QR codes)
app.get('/verify-application', (req, res) => {
    ...
    if (!appId) {
        return res.send(`<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Application Verification</title>
        ...
        <div class="container"><h1>❌ Invalid Link</h1><p>No application ID provided. Please scan the QR code from the original admission form.</p></div>
        </body></html>`);
    }
    
    // Find student by application ID (try both stored applicationId and computed appId)
    const students = readData('students.json') || [];
    const year = new Date().getFullYear();
    const student = students.find(s => {
        if (s.applicationId === appId) return true;
        const serial = String(s.id || s.rollNo || '').slice(-5).padStart(5, '0');
        const computedId = 'GCE-' + year + '-' + serial;
        if (computedId === appId) return true;
        // Try with student's admission year
        if (s.admissionDate) {
            const admYear = new Date(s.admissionDate).getFullYear();
            const computedId2 = 'GCE-' + admYear + '-' + serial;
            if (computedId2 === appId) return true;
        }
        return false;
    });
    ...
    if (!student) {
        ...
        <div class="container"><div class="badge">✗ Not Verified</div><h1>Invalid Application</h1>
        <p>No admission record found for Application ID: <strong>${appId}</strong></p>
        <p>Please verify the document with the institute administration.</p></div>
        </body></html>`);
    }
    ...
    res.send(`<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Application Verified - ${instituteName}</title>
    ...
    <div class="container">
        <div class="header">
            ...
            ${photo}
            <h1>${instituteName}</h1>
            <p>Admission Application Verification</p>
        </div>
        <div class="details">
            <div class="detail-item full-width"><label>Student Name</label><span>${student.name || 'N/A'}</span></div>
            <div class="detail-item"><label>Application ID</label><span>${appId}</span></div>
            <div class="detail-item"><label>Roll Number</label><span>${student.rollNo || 'N/A'}</span></div>
            <div class="detail-item"><label>Course</label><span>${student.course || 'N/A'}</span></div>
            <div class="detail-item"><label>Batch</label><span>${student.batch || 'N/A'}</span></div>
            <div class="detail-item"><label>Admission Date</label><span>${student.admissionDate || 'N/A'}</span></div>
